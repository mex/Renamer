...
class Renamer {
	...
	private void runScript() {
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (fileName.length() > 14 && fileName.substring(0, 14).equals("The.Daily.Show")) {
				String date = fileName.substring(15, 25).replace(".", "-");
				String title = fileName.substring(26).split(".HDTV")[0].replace(".", " ");
				try {
					String newName = "The Daily Show - " + date + " - " + title + "." + this.getExt(files[i].getName());
					this.appendOutput("Renaming to '" + newName + "'...");
					this.renameFile(files[i], newName);
				} catch (Exception e) {
					this.appendOutput("Error occurred: " + e.getMessage());
				}
			} else if (fileName.length() > 15 && fileName.substring(0, 15).equals("David.Letterman")) {
				String date = fileName.substring(16, 26).replace(".", "-");
				String title = fileName.substring(27).split(".HDTV")[0].replace(".", " ");
				try {
					String newName = "David Letterman - " + date + " - " + title + "." + this.getExt(files[i].getName());
					this.appendOutput("Renaming to '" + newName + "'...");
					this.renameFile(files[i], newName);
				} catch (Exception e) {
					this.appendOutput("Error occurred: " + e.getMessage());
	...
	private String fetchTitle(String body, int s, int e) {
		...
		for (int i = 0; i < lines.length; i++) {
			if (lines[i].matches("<div>S" + s + ", Ep" + e + "</div>")) {
				String se = "<img width=\"120\" class=\"zero-z-index\" alt=\"(.*?)\" src=\"(.*?)\">";
				if (!Pattern.matches(se, lines[i-1])) return "";
				Pattern p = Pattern.compile(se);
				Matcher m = p.matcher(lines[i-1]);
				m.find();
				return " - " + this.cleanTitle(m.group(1));
			}
		}
		return "";
	}
